...
bool is_valid_boardsize(uint8_t width, uint8_t height) {
  ...
  if (width <= height/UINT8_MAX) return false;
...
static inline char read_non_space(FILE *F) {
  char c;
  do c = fgetc(F);
  while (isspace(c));
  return c;
